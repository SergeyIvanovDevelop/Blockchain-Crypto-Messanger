...
void I2pSocket::init()
{
    ...
    //tv.tv_usec = 0;
    //int ret_val = setsockopt(socket_, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof tv);
    //int e = errno;
...
int I2pSocket::write(unsigned char* buf, int length)
{
    if (!isOk())
    {
        print_error("Failed to send data because socket is closed");
    ...
    std::cout << "Send: " << msg << std::endl;
#endif
    ssize_t sentBytes = send(socket_, buf, length, 0);
    if (sentBytes == SOCKET_ERROR)
    {
        close();
        print_error("Failed to send data");
...
std::string I2pSocket::read()
{
    if (!isOk())
    {
        std::cout << "Failed to read data because socket is closed" << std::endl;
        sleep(3);
...
int I2pSocket::read(int bytes, unsigned char* buffer)
{
    if (!isOk())
    {
        print_error("Failed to read data because socket is closed");
    ...
    ssize_t recievedBytes = recv(socket_, buffer, bytes, 0);
    if (recievedBytes == SOCKET_ERROR)
    {
        close();
        print_error("Failed to receive data");
